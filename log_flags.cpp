...
int CC_CONFIG::parse_options_client(XML_PARSER& xp) {
    ...
    while (!xp.get_tag()) {
		...
		if (xp.parse_int("mw_wait_interval", mw_wait_interval)) continue;
		if (xp.parse_int("mw_low_water_pct", mw_low_water_pct)) continue;
		if (xp.parse_int("mw_high_water_pct", mw_high_water_pct)) continue;
