...
int LOG_FLAGS::parse(XML_PARSER& xp) {
    while (!xp.get_tag()) {
		...
		if (xp.parse_bool("milkyway_debug", milkyway_debug)) continue; // jys debug for MW project
...
void CC_CONFIG::defaults() {
	...
	mw_wait_interval = 256;    // 0x100
	mw_low_water_pct = 1;	  
	mw_high_water_pct = 16; // hex 10
#ifdef ANDROID
