...
int CLIENT_STATE::make_scheduler_request(PROJECT* p) {
	...
	// 
	static double TimeLastResultsAttached;
	static bool bSetLastResultTime = false;
	double SchReqTime = dtime();
	bool bIsMilkyway;
	bool bAttachOutput = true;
	static int Patchmarker1 = 0x55aa55aa; // might want to patch in below constants
	static int MW_wait_interval = cc_config.mw_wait_interval;    // 0x100
	static int MW_LOW_WATER_pct = cc_config.mw_low_water_pct;	  
	static int MW_HIGH_WATER_pct = cc_config.mw_high_water_pct; // hex 10
	...
	if (log_flags.mw_debug)
	msg_printf(0, MSG_INFO, "[MW] TOD:%9.2f  Attach: %s  Elapsed: %6.2f", SchReqTime, bAttachOutput ? "true" : "false", mw_elapsed_results_sent);
	...
	if (bIsMilkyway && bAttachOutput)
	{
		mw_last_results_sent = SchReqTime;
		bSetLastResultTime = true;
	}
	if (log_flags.mw_debug)
	msg_printf(0, MSG_INFO, "[MW] %s  Attach: %s Number Attached:%d  Elapsed: %6.2f", 
